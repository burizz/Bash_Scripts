...
if [ ! -f $DWNLD_DIR/cf_uaac_gem_cache_$cf_uaac_version.tar.gz ]; then \
mkdir $LTS_VERS_DIR/gemrepo_$cf_uaac_version/ && \
cd $LTS_VERS_DIR/gemrepo_$cf_uaac_version/ && \
HTTP_PROXY="<PROXY_URL:PORT>" gem install cf-uaac --platform x86_64-linux -i gemrepo --no-document && \
tar cvzf cf_uaac_gem_cache_$cf_uaac_version.tar.gz gemrepo/cache/ && \
cp $LTS_VERS_DIR/gemrepo_$cf_uaac_version/cf_uaac_gem_cache_$cf_uaac_version.tar.gz $LTS_VERS_DIR/ && \
cp $LTS_VERS_DIR/gemrepo_$cf_uaac_version/cf_uaac_gem_cache_$cf_uaac_version.tar.gz $DWNLD_DIR/ && rm -rf $LTS_VERS_DIR/gemrepo_$cf_uaac_version \
; fi

# Download cf_uaac source tar instead of ruby gem
#
...
if [ ! -f $DWNLD_DIR/cfops_binary-$cfops_version ]; then \
curl -v -k -L -o $DWNLD_DIR/cfops_binary-$cfops_version $(curl -s -k -L https://api.github.com/repos/pivotalservices/cfops/releases/latest | grep "browser_download_url" | grep -Eo "(http|https)://[\da-z./?A-Z0-9\D=_-]*") && \
cp $DWNLD_DIR/cfops_binary-$cfops_version $LTS_VERS_DIR \
; fi
