...
# 1. Get file and pattern as arguments with argv and additional option   -- DONE
# 2. Find and open file to search through
# 3. Traverse lines and if $PATTERN is matched return the line that has $PATTERN
# 4. Add options for case insesitivity (-i), for recursive traversion of directories (-R, -r, --recursive)
...
print "This is an implementation of the grep program in Python : "

def grep():
    """ Check for pattern in a file and print the line if pattern is matched """
    print "%s is searching for %s in %s" % (script, pattern, path)
    open_file(path)

def grep_with_opts():
    """ Adds additional functionality to grep, i.e. case insesitivity, recursive directory search, etc """
    print "WITH OPTION !"
    print "%s is searching for %s in %s" % (script, pattern, path)

def check_if_opts(*args):
    """ Check if additional cmd line args are passed to grep """
    for arg in args:
...
def match_patterh(pattern):    # Not sure if that will be needed or if matching with with "if $PATTERN in line" will work
    pass
